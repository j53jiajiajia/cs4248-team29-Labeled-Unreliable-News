"""
Author: Daren (Team 29)
Date: Mar 30, 2024
Description: Python script to extract hidden state output from finetuned DistilBERT
"""
...
from model1 import DistilBERTClassifier         # Import our finetuned DistilBERT model
from model1 import read_data, tokenize_data     # Import functions to tokenize the data
...
def get_hidden_state(data_loader, data_type):
    ...
    initialise_data = {"BERT" + str(i): [] for i in range(HIDDEN_LAYER_NODES)}
    ...
    output_directory = os.path.join(os.path.dirname(os.getcwd()), "data_code", "dataset", "DistilBERT", str(MAX_LENGTH))
