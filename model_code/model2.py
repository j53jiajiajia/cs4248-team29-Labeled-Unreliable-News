...
Date: Mar 28, 2024
Description: Python script to train DistilBERT model on classification (without attention masks)
...
class DistilBERTClassifier(nn.Module):
    ...
    def forward(self, input_ids):
        bert_output = self.distil_bert(input_ids=input_ids)
...
def tokenize_data(data):
    ...
    train_loader = create_dataloader(train_tokens, data[1], isTrain=True)
...
def create_dataloader(X_tokens, y_labels, isTrain=False, batch_size=32):
    """
    ...
    :param isTrain:    True if data and labels belong to training set; False otherwise
    ...
    # Perform oversampling for training set
    if isTrain:
        smote = SMOTE(random_state=4248)
        X_seq, y_labels = smote.fit_resample(X_seq.numpy(), y_labels.numpy())
        X_seq, y_labels = torch.tensor(X_seq), torch.tensor(y_labels)

    dataset = TensorDataset(X_seq, y_labels)
...
if __name__ == '__main__':
    ...
    data = read_data()
    train_loader, val_loader, test_loader = tokenize_data(data)
